...
class WebRTCManager {
  constructor() {
    ...
    this.onParticipantsListCallback = null;
  ...
  setupSocketListeners() {
    ...
    this.socket.on("server-message", (message) => {
      ...
      if (message.type === "welcome") {
        this.log(
          `服务器欢迎消息: ${message.message}, Socket ID: ${message.socketId}`
        );
      }
    });
  ...
  async connectToPeer(targetUserId, isInitiator = false) {
    ...
    this.peer.on("stream", (stream) => {
      this.log(`收到来自 ${targetUserId} 的远程媒体流`);
      this.remoteStream = stream;

      // 只调用回调函数，让meeting.js处理视频显示
      if (this.onRemoteStreamCallback) {
        this.log(`调用远程流回调函数，用户ID: ${targetUserId}`);
        this.onRemoteStreamCallback(targetUserId, stream);
      } else {
        this.error("未设置远程流回调函数");
      }
    });
  ...
  combineStreams() {
    ...
    // 如果已经创建了对等连接，需要重新创建连接
    // 注意：SimplePeer库不支持getSenders方法，因此无法使用removeTrack
    // 如果需要更新流，需要销毁并重新创建连接
    if (this.peer && this.isConnected) {
      this.log("流已更新，但SimplePeer不支持动态更改轨道，如需更新请重新连接");
    }
  ...
  setCallbacks(callbacks) {
    ...
    if (callbacks.onUserJoined)
      this.onUserJoinedCallback = callbacks.onUserJoined;
    if (callbacks.onUserLeft) this.onUserLeftCallback = callbacks.onUserLeft;
    if (callbacks.onParticipantsList)
      this.onParticipantsListCallback = callbacks.onParticipantsList;
  }
  ...
  sendPointCloudData(positions, colors, useBinary = true) {
    ...
    // 检查数据通道缓冲区状态
    if (this.peer.bufferSize > 5000000) {
      // 降低缓冲区限制到5MB
      console.warn(
        `[WebRTC] 数据通道缓冲区已满(${
          this.peer.bufferSize / 1000000
        }MB)，跳过此帧点云数据`
      );
      return true;
    }
    ...
    // 激进的降采样：只保留10%的点
    const sampleRate = 10;
    const sampledPositions = new Float32Array(
      Math.floor(posArray.length / sampleRate)
    );
    const sampledColors = new Float32Array(
      Math.floor(colArray.length / sampleRate)
    );
    ...
    console.log(
      `[WebRTC] 点云数据降采样: ${posArray.length / 3} -> ${
        sampledPositions.length / 3
      } 个点`
    );
    ...
    try {
      if (useBinary) {
        ...
        const combinedBuffer = new Float32Array(
          sampledPositions.length + sampledColors.length
        );
  ...
  handleDataChannelMessage(event) {
    try {
      ...
      if (typeof event === "string") {
        ...
        console.log(
          "[WebRTC] 收到二进制数据, 长度:",
          event.byteLength || event.buffer.byteLength
        );
        ...
        try {
          ...
          const dataStr = textDecoder.decode(
            event instanceof ArrayBuffer ? event : event.buffer
          );
          // 检查是否像JSON字符串
          if (dataStr.trim().startsWith("{") && dataStr.trim().endsWith("}")) {
            ...
            try {
              ...
              console.log(
                `[WebRTC] 成功从二进制数据解析JSON, 类型: ${data.type}`
              );
              ...
              console.log(
                "[WebRTC] 无法将二进制数据解析为JSON，尝试作为点云数据处理"
              );
          ...
          console.log(
            "[WebRTC] 无法将二进制数据转换为字符串，尝试作为点云数据处理"
          );
        ...
        if (isJsonData) {
          ...
          try {
            ...
            console.log(
              `[WebRTC] 从二进制数据提取: 位置数组长度=${positions.length}, 颜色数组长度=${colors.length}`
            );
      ...
      switch (data.type) {
        case "pointCloudData":
          ...
          console.log(
            `[WebRTC] 接收到点云数据: 位置数组长度=${data.positions.length}, 颜色数组长度=${data.colors.length}`
          );
          ...
          if (positions.length !== colors.length) {
            console.error(
              `[WebRTC] 点云数据不匹配: 位置长度(${positions.length}) 不等于颜色长度(${colors.length})`
            );
            return;
          }
        ...
        case "viewModeChange":
          ...
          if (window.handleRemoteViewModeChange) {
            console.log(
              `[WebRTC] 调用全局处理函数 handleRemoteViewModeChange 切换到 ${data.mode} 模式`
            );
            try {
            ...
            console.warn(
              "[WebRTC] 收到视图模式切换请求，但未找到 handleRemoteViewModeChange 全局函数"
            );
          }
          break;
  ...
  setPointCloudMode(enabled) {
    ...
    console.log(
      `[WebRTC] 当前点云回调函数状态: ${
        this.onPointCloudDataCallback ? "已设置" : "未设置"
      }`
    );
  ...
  // 添加setVideoMode方法，供meeting.js调用
  setVideoMode(mode) {
    console.log(`[WebRTC] 设置视频模式: ${mode}`);

    // 如果是点云模式，启用点云数据传输
    if (mode === "pointCloud") {
      this.setPointCloudMode(true);
    } else {
      this.setPointCloudMode(false);
    }

    // 向远程对等方发送模式变更通知
    this.sendViewModeChange(mode);
  }
}
